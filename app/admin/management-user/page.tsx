  ...
  const updateUserRole = async (id: string, role: string, team: string) => {
    ...
    try {
      ...
      if (res.ok) {
        setUsers((prev) =>
          prev.map((u) =>
            u.id === id ? { ...u, role, team, access: role === "staff" && team === "pm" ? u.access : [] } : u
          )
        );
  ...
  const toggleTeam = async (id: string, selectedAccess: string) => {
    ...
    try {
      ...
      const res = await fetch(`/api/admin/users/${id}/team`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ team: newAccess }),
      });
              ...
              <tr key={user.id} className="border-b border-gray-200 dark:border-gray-700">
                ...
                <td className="px-5 py-3 text-sm text-gray-700 dark:text-gray-200">
                  <select
                    ...
                    value={user.role}
                    onChange={(e) =>
                      updateUserRole(user.id, e.target.value, TEAMS_BY_ROLE[e.target.value][0])
                    }
